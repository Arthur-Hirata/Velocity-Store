...
def precoFinal(userId):
    with sqlite3.connect("banco-users.db") as conexao:
        cursor = conexao.cursor()
        cursor.execute("SELECT buylist FROM users WHERE id=?", (userId))
        result = cursor.fetchone()
    
    if result and result[0]:
        lista_carrinho = json.loads(result[0])
        total = sum(float(produto.get('preco', 0))for produto in lista_carrinho)
        return total
    return 0.0
...
def mostrar_lista():
    ...
    total_carrinho = precoFinal(user_Id)
    return jsonify({"produtos":lista_carrinho, "preco_final" : total_carrinho})
...
def remover_item(id):
    ...
    with sqlite3.connect("banco-users.db") as conexao:
        ...
        cursor.execute("SELECT buylist FROM users WHERE id=?", (user_Id))
        result = cursor.fetchone()
        total_preco = precoFinal(result)
        conexao.commit()
        
    return ({"status":"removido", "preco_final" : total_preco}), 200         
...
def pegarCredenciais():
    ...
    user_name, user_list, user_role = resultado
    return jsonify({"mensagem": "usuário encontrado", "nome" : user_name, "buylist" : user_list, "role" : user_role})
